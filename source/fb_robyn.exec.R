...
OutputCollect$xDecompAgg[solID == select_model & !is.na(mean_spend)
                         , .(rn, coef,mean_spend, mean_response, roi_mean
                             , total_spend, total_response=xDecompAgg, roi_total, solID)] #check media summary for selected model
...
AllocatorCollect <- robyn_allocator(InputCollect = InputCollect
                                    ...
                                    , scenario = "max_historical_response" # options are "max_historical_response", "max_response_expected_spend"
...
# report_aggregated.csv, aggregated decomposition per independent variable of all selected reporting model 
...
Response1 <- robyn_response(robyn_object = robyn_object
                            #, select_run = 1 # 0 is initial model, 1 is first refresh etc. Default to the latest refresh
