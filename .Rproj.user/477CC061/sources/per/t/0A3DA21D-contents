...
AllocatorCollect <- robyn_allocator(
  ...
  , channel_constr_low = c(0.7, 0.7, 0.7, 0.7, 0.7) # must be between 0.01-1 and has same
  #length and order as paid_media_vars
  , channel_constr_up = c(1.2, 1.5, 1.5, 1.5, 1.5) # not recommended to 'exaggerate' upper
  #bounds. 1.5 means channel budget can increase to 150% of current level

  # , expected_spend = 100000 # specify future spend volume. only applies when scenario =
  # # "max_response_expected_spend"
  # , expected_spend_days = 90 # specify period for the future spend volumne in days.
  # # only applies when scenario = "max_response_expected_spend"
...
AllocatorCollect <- robyn_allocator(
  ...
  #same length and order as paid_media_vars
  , channel_constr_up = c(1.2, 1.5, 1.5, 1.5, 1.5) # not recommended to 'exaggerate' upper
  #bounds. 1.5 means channel budget can increase to 150% of current level
